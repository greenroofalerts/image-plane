...
print("top 20 cluster sizes:", [len(c) for c in clusters[:20]])
...
json.dump({"residual_gps_no_job": len(gps_no_job), "far_gt_2km": len(far),
           ...
           "top20_sizes": [len(c) for c in clusters[:20]],
           "top20_photo_count": top20_photos,
           "top20_pct_of_tail": round(100*top20_photos/len(far), 1) if far else None},
          open(os.path.join(G, "f2_residual_summary_v2.json"), "w"), indent=1)
print("wrote grind/f2_residual_summary_v2.json")
